var order = require('./byte_order.js');
...
IMService.prototype.connect = function () {
    ...
    this.socket = wx.connectSocket({ //创建webSocket连接
          url: url
        })
    this.socket.binaryType = 'arraybuffer';
    
    var self = this;
    wx.onSocketOpen(function(evt) { //监听webSocket打开事件
        self.onOpen();
    });

    wx.onSocketError( function(event) { //监听webSocket错误
        self.onError(event);        
    });    
};
...
IMService.prototype.onOpen = function () {
    ...
    var self = this;
    wx.onSocketMessage( function(message) { //监听webSocket接受到服务器消息事件
		// console.log('onMessage',message);
        self.onMessage(message.data)
    });
    wx.onSocketClose( function() { //监听webSocket关闭
...
IMService.prototype.handleMessage = function(msg) {
    ...
    if (cmd == IMService.MSG_IM) {
        ...
        console.log("身份认证状态 status:" + status);
        if (status != 0) {
...
IMService.prototype.send = function (cmd, body, nonpersistent) {
	// console.log('send',cmd,body,nonpersistent)
    if (this.socket == null) {
        return false;
    }
    if (this.connectState != IMService.STATE_CONNECTED) {
    ...
    buf.set(body, pos);
    pos += body.length;
	console.log('sendData',buf.buffer)
    wx.sendSocketMessage({ //通过webSocket连接发送数据
      data:buf.buffer,
	  success:(res)=>{
		  // console.log('监听发送结果',res)
	  },fail:(err)=>{
		  console.log('监听发送失败结果',res)
	  }
    })
...
IMService.prototype.leaveRoom = function(roomID) {
	console.log('leaveRoom',this.roomID,'roomID',roomID)
    if (roomID != this.roomID || roomID == 0) {
        return;
    }
