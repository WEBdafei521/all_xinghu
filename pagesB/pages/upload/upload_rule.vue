		...
		<view class="l-type-list">
			...
			<view class="l-type-item" v-for="(item,index) of typeListInfo" :key="index">
				...
				<view class="l-type-item-price l-my-flex-bw">
					<view class="l-my-flex-bw">
						<view>原价</view>
						<view class="l-price-money"><text style="color: #E93340;">￥</text>  <input class="l-inp" type="text" v-model="item.oldPrice" placeholder="0.00"/> </view>
					</view>
					<view class="l-my-flex-bw">
						<view>现价</view>
						<view class="l-price-money"><text style="color: #E93340;">￥</text>  <input class="l-inp" type="text" v-model="item.newPrice" placeholder="0.00"/></view>
					</view>
				</view>
				...
				<view class="l-type-item-price l-my-flex-bw">
					<view class="l-my-flex-bw">
						<view>库存</view>
						<view class="l-price-kucun"> <input class="l-inp" type="text" v-model="item.stock" placeholder="0"/> </view>
					</view>
				</view>
				<!-- 预览图 -->
		...
		<u-popup v-model="show_setting" mode="center" border-radius="14">
				<view class="l-setting">
					...
					<view class="l-setting-old-price l-my-flex-start">
						...
						<view class="l-old-price-value"><input type="text" v-model="l_setting_old_price" placeholder="￥0.00" placeholder-style="#E93340;"/></view>
					</view>
					
					<view class="l-setting-old-price l-my-flex-start">
						<view class="l-old-price">现价</view>
						<view class="l-old-price-value"><input type="text" v-model="l_setting_new_price" placeholder="￥0.00" placeholder-style="#E93340;"/></view>
					</view>
	...
	export default {
		...
		methods: {
		  ...
		  btnClick() {
					...
					for(var item of this.list0){
						if(this.list1.length != 0){
							for(var idx of this.list1){
								...
								obj.stock = ""
								obj.name = item.size
								obj.name_value = idx.size
								obj.value1 = this.typeList[0].size
								obj.value2 = this.typeList[1].size
								arr.push(obj)
							}
						}else{
								var obj = {}
							...
							uni.uploadFile({
							    ...
							    success: function (uploadFileRes) {
										...
										if(uploadFileRes.statusCode==200){
											that.typeListInfo[index].url = obj
			...
			// 弹出框 选择 规格 确定按钮
			comfirmSelect(){
			  if(this.typeList.length == 1 && this.active_index.length == 2){
					uni.showToast({
						title:"只能再选一种",
						icon:'none'
					})
				...
				var arr = this.typeList.concat(typeList)
				this.typeList = arr
				this.active_index = []
			},
			// 显示批量设置价格和库存
